...
| **IAM 사용자 정책** | `IAMAdvancedAccess` + `IAMBasicAccess` + `SafePowerUser` + `DenyAllWithoutMFA` 등 | IAM Role/Policy/OIDC Provider 생성 가능 가정. **MFA 필수** |
...
학생 IAM 사용자(`DenyAllWithoutMFA` 정책) 운영 시 인증 패턴:
...
- 2026-05-04 PIVOT: 학생 계정 SCP 제약 발견 후 architecture 전면 재설계 (A2 트랙)
  ...
  - IAM 정책 확인: IAMAdvancedAccess + IAMBasicAccess + SafePowerUser + DenyAllWithoutMFA — IAM Role 생성 가능 가정 (apply 시 검증 필요)
