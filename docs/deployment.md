...
|---|---|---|---|---|
| `tracker-prod` (단일) | **t3.xlarge 16GB** | redis + crawler + detection + api + dashboard | 워킹셋 1.7~3.3GB / 16GB (~21%) | 22 (SSH), 80 (dashboard), 8080 (api). redis 6379는 docker network 내부 only |
...
.github/workflows/deploy.yml
  ...
  3. deploy   (environment: production, concurrency: deploy-prod / serial)
        │
        ▼ scp-action → /opt/app/compose.prod.yml 업로드
        ▼ ssh-action → docker login ghcr → compose pull → compose up -d
        ▼ healthcheck 폴링 60초 (5 컨테이너: redis crawler detection api dashboard)
        │
   ┌────┴────┐
...
`deploy.yml`의 deploy 잡 마지막 단계가 30초간 healthcheck를 폴링합니다.

| 결과 | 동작 |
|---|---|
| 4개 서비스 모두 `healthy` | `/opt/app/IMAGE_TAG`에 새 SHA 기록 → `exit 0` |
| 30초 내 한 개라도 실패 | 직전 `IMAGE_TAG` 파일 값(이전 성공 SHA)으로 `IMAGE_TAG` env 재설정 → `compose up -d` 재기동 → `exit 1` (GHA UI 실패 표시) |
